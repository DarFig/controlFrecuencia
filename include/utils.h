...
void run_tool(const char* tool_path){
    int pidd = fork();
    if (pidd < 0) exit(1);
    if(pidd == HIJO){ 
        execl(tool_path, "",NULL); exit(0);
    }
}
...
void set_governor(int _governorFlag){
    if(actualGovernor != _governorFlag){
        if(_governorFlag == POWERSAVE){
            ...
            run_tool("./tools/powersave.sh");
            sleep(2); 

        }else
        if(_governorFlag == USERSPACE){
            
            run_tool("./tools/userspace.sh");
            sleep(2);

        }else
        if(_governorFlag == ONDEMAND){
            
            run_tool("./tools/ondemand.sh");
            sleep(2);

        }else
        if(_governorFlag == PERFORMANCE){
             
            run_tool("./tools/performance.sh");
            sleep(2);    
...
void set_frequency(int _frequencyFlag){
    if(actualFrequency != _frequencyFlag){
        if(_frequencyFlag == UN_GHZ){
            ...
            run_tool("./tools/unghz.sh");
            sleep(1);

        }else
        if(_frequencyFlag == UNO_CINCO_GHZ){
            
            run_tool("./tools/unoconcincoghz.sh");
            sleep(1);

        }else
        if(_frequencyFlag == DOS_GHZ){
            
            run_tool("./tools/dosghz.sh");
            sleep(1);
